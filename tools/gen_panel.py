...
:root {
  ...
  /* Mode washes. Desaturated on purpose: operators sit in front of this all
     day, and saturated colour would both fatigue them and compete with the Not
     Ready pills, which must stay the loudest thing on screen. All keep body
     text well over 7:1 contrast.
     Base is deliberately untinted -- it is the ordinary state, and a screen
     that is always coloured says nothing by being coloured. */
  --bg-summit: #f2dede;
  --edge-summit: #b3403f;
  --bg-standby: #f6eeda;
  --edge-standby: #8a6410;
...
body[data-mode="standby"] { background: var(--bg-standby); }
body[data-mode="standby"] main { border-top: 3px solid var(--edge-standby); }
body[data-mode="standby"] .mode-val { color: var(--edge-standby); }
